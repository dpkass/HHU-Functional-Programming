...
; added from intro (possibly simply misplaced)
(mm inc [1 2 3])
(mm inc '(1 2 3))
(mm inc #{1 2 4})
...
; works if first element isn't :none...
(defn first [coll]
  (reduce #(if (= %1 :none) %2 %1) :none coll))

(first [1 2 3])


(defn map [f coll]
  (reduce #(cons (f %2) %1) (empty coll) (reverse coll)))

(map inc (range 10))


(defn filter [f coll]
  (reduce #(if (f %2) (cons %2 %1) %1) (empty coll) (reverse coll)))

(filter #(< 4 %) (range 10))


(defn concat [coll1 coll2]
  (reduce #(conj %1 %2) (vec coll1) coll2))
